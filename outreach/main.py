...
and sends them a message on Twitter and/or LinkedIn.
"""
import sys
from playwright.sync_api import sync_playwright

from config import (
    PRODUCT_HUNT_TOKEN, TWITTER_USERNAME, TWITTER_PASSWORD,
...
def validate_config():
    ...
    if not TWITTER_USERNAME or not TWITTER_PASSWORD:
        missing.append("TWITTER_USERNAME / TWITTER_PASSWORD")
...
def run():
    ...
    with sync_playwright() as p:
        ...
        for maker in targets:
            ...
            if not linkedin_url:
                print(f"    No LinkedIn link found on PH profile")
                continue

            key = f"linkedin:{linkedin_url}"
            if key in sent_log:
                print(f"    ↩  Already messaged on LinkedIn, skipping")
                continue
